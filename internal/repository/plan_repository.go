...
func (r *PlanRepository) ExecutionCount(id uint) (int64, error) {
	var count int64
	err := r.db.Model(&model.ControlExecution{}).Where("feeding_plan_id = ? AND status <> ?", id, "cancelled").Count(&count).Error
	return count, err
}

func (r *PlanRepository) LatestApprovedForPond(pondID uint) (model.FeedingPlan, error) {
	var plan model.FeedingPlan
	err := r.db.Preload("Pond").Where("pond_id = ? AND status = ?", pondID, "pending").Order("version DESC, updated_at DESC").First(&plan).Error
	return plan, err
}
